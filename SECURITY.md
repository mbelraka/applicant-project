...
| **A05 Misconfiguration** | Security headers (referrer policy, framing, Permissions-Policy, optional HSTS); CORS allowlist — **production mode refuses `CORS_ORIGIN=*` at startup**; `TRUST_PROXY` behind reverse proxies so rate limits use real client IPs. |
| **A06 Vulnerable components** | Keep `npm` and Maven dependencies updated; run `npm run security:audit` and Spring OWASP Dependency-Check in CI. |
